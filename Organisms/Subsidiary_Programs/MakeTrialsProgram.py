...
class MakeTrialsProgram:
    def __init__(self,cluster_makeup,pop_size,generations,no_offspring_per_generation,creating_offspring_mode,crossover_type,mutation_types,chance_of_mutation,r_ij,vacuum_to_add_length,Minimisation_Function,surface_details,epoch_settings,cell_length,memory_operator_information,predation_information,fitness_information,ga_recording_information,force_replace_pop_clusters_with_offspring,user_initialised_population_folder,rounding_criteria,print_details,no_of_cpus,dir_name,NoOfTrials,Condense_Single_Mention_Experiments,JobArraysDetails,making_files_for,finish_algorithm_if_found_cluster_energy=None,total_length_of_running_time=None,no_of_packets_to_make=None): 
        ...
        self.check_mem_and_mem_per_cpu()
        self.check_inputs()
        self.MakeTrials()
        print('Created Multiple Trials files for Slurm')

    def check_mem_and_mem_per_cpu(self,mem=None,mem_per_cpu=None):
        if 'mem' not in self.JobArraysDetails:
            self.JobArraysDetails['mem'] = None
        if 'mem-per-cpu' not in self.JobArraysDetails:
            self.JobArraysDetails['mem-per-cpu'] = None
        mem = self.JobArraysDetails['mem']
        mem_per_cpu = self.JobArraysDetails['mem-per-cpu']
    ...
    def make_individual_trials(self):
        ...
        for trial_no in range(1,self.NoOfTrials+1):
            ...
            make_submitSL(path_to,self.JobArraysDetails['project'],self.JobArraysDetails['time'],self.JobArraysDetails['nodes'],self.JobArraysDetails['ntasks_per_node'],self.JobArraysDetails['mem'],self.JobArraysDetails['mem-per-cpu'],self.JobArraysDetails['partition'],self.JobArraysDetails['email'],self.JobArraysDetails['python version'])
    ...
    def make_slurm_jobarray_files(self):
        ...
        if self.making_files_for == 'slurm_JobArrays_full':
            make_mass_submitSL_full(path_to,self.JobArraysDetails['project'],self.NoOfTrials,self.JobArraysDetails['time'],self.JobArraysDetails['nodes'],self.JobArraysDetails['ntasks_per_node'],self.JobArraysDetails['mem'],self.JobArraysDetails['mem-per-cpu'],self.JobArraysDetails['partition'],self.JobArraysDetails['email'],self.RunMinimisation_filename,self.JobArraysDetails['python version'])
            ...
            print('======================================================================')
            print('======================================================================')
            make_mass_submitSL_packets(path_to,self.JobArraysDetails['project'],self.NoOfTrials,self.no_of_packets_to_make,self.JobArraysDetails['time'],self.JobArraysDetails['nodes'],self.JobArraysDetails['ntasks_per_node'],self.JobArraysDetails['mem'],self.JobArraysDetails['mem-per-cpu'],self.JobArraysDetails['partition'],self.JobArraysDetails['email'],self.RunMinimisation_filename,self.JobArraysDetails['python version'])
        else:
