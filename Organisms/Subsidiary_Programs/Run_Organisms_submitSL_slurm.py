#!/usr/bin/env python3
'''
Geoffrey Weal, Run_Organisms_submitSL_slurm.py, 10/02/2021
...
print('###########################################################################')
print('###########################################################################')
print('Run_Organisms_submitSL_slurm.py')
print('###########################################################################')
print('This program is designed to submit all your submit.sl and mass_submit.sl scripts appropriately to slurm.')
print('###########################################################################')
print('###########################################################################')
        ...
        for line in mass_submitSL:
            if '#SBATCH --array=' in line:
                ...
                if worked_successfully:
                    ...
                    exit ('========================================================')         
    print('Error in def get_number_to_trials_that_will_be_submitted_by_mass_submitSL, in Run_Organisms_submitSL_slurm.py script, found in the folder SubsidiaryPrograms in this GA program.')
...
print('*****************************************************************************')
print('*****************************************************************************')
print('Submitting submit.sl and mass_submit.sl scripts to slurm.')
...
for (dirpath, dirnames, filenames) in os.walk(path):
    dirnames.sort()
    got_submitSL      = 'submit.sl'      in filenames
    got_mass_submitSL = 'mass_submit.sl' in filenames
    if got_submitSL or got_mass_submitSL:
        ...
        while True:
            ...
            if reached_max_jobs:
                ...
                break
        # Submitting a submit.sl or mass_submit.sl file
        if got_submitSL and got_mass_submitSL:
            print('WARNING: I found both a submit.sl and mass_submit.sl file in: 'str(dirpath))
            print('You only want to have either a submit.sl or mass_submit.sl file, as we only want to submit one of these submit files to slurm.')
            print('To avoid issues, will pass here and not submit this job. This is something for you to look at.')
            dirnames[:] = []
            filenames[:] = []
            continue
        if got_submitSL:
            submit_filename = 'submit.sl'
        elif got_mass_submitSL:
            submit_filename = 'mass_submit.sl'
        # submit the jobs
        os.chdir(dirpath)
        ...
        while True:
            if error_counter == number_of_consecutive_error_before_exitting:
                ...
                proc = subprocess.Popen(submitting_command+submit_filename, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                if not proc.wait() == 0:
                    error_counter += 1
                    if error_counter == number_of_consecutive_error_before_exitting:
                        print('----------------------------------------------')
                        ...
                        print('Run_Organisms_submitSL_slurm.py will retry submitting this job to slurm after '+str(time_to_wait_before_next_submission_due_to_temp_submission_issue)+' seconds of wait time')
                        print('----------------------------------------------')
        ...
        if error_counter == number_of_consecutive_error_before_exitting:
            print('----------------------------------------------')
            print('Reached the total number of consecutive errors with: ')
            print(dirpath)
            print('Will continue on with submissions and let you figure out what the issue with this submissino to slurm is')
            print('----------------------------------------------')
...
if error_counter == number_of_consecutive_error_before_exitting:
    print('----------------------------------------------')
    print()
    print('"Run_Organisms_submitSL_slurm.py" will finish WITHOUT HAVING SUBMITTED ALL JOBS.')
    print()
    print('*****************************************************************************')
