...
def compared_cluster_interatomic_distances(cluster_interatomic_distances_1, cluster2):
	for index1 in range(len(cluster2)):
		for index2 in range(index1+1,len(cluster2)):
			cluster2_interatomic_distance = round(get_distance(cluster2[index1],cluster2[index2]),4)
...
def process_databases(number_of_clusters_per_database, sort_clusters_by='cluster_energy'):

	# ----------------------------------------------------------------------------------------------------------------------------

	Recorded_Data_foldername = 'Recorded_Data'
	database_name = 'GA_Recording_Database'

	print('========================================================')
	print('Getting all paths to databases for your trials')
	paths_to_databases = []
	for root, dirs, files in os.walk('.'):
		if (Recorded_Data_foldername in dirs) and ('Trial' in root.split('/')[-1]):
			paths_to_databases.append(root)
			dirs[:] = []
			files[:] = []
	paths_to_databases.sort()
	...
	clusters_info = []
	pbar = trange(len(paths_to_databases), unit='database')
	for path_to_database_index in pbar:
		path_to_database = paths_to_databases[path_to_database_index]
		pbar.set_description(path_to_database.replace(current_directory, ''))
		print('Currently processing '+str(path_to_database))

		if os.path.exists(path_to_database+'/'+Recorded_Data_foldername+'/'+database_name+'.db'):
			db = connect (path_to_database+'/'+Recorded_Data_foldername+'/'+database_name+'.db')
		else:
			exit('Error in '+str(path_to_database)+'. You need to be inside the Recorded_Data folder than contains the '+str(Recorded_Data_foldername)+'/'+str(database_name)+'.db database file.')
		...
		for row in db.select():
			...
			if row.id%10000 == 0:
				print(row.id)
			clusters_info.append((path_to_database_index, row.id, row.get(sort_clusters_by)))
		print('========================================================')

	# ----------------------------------------------------------------------------------------------------------------------------

	clusters_info.sort(key=lambda x:x[2])

	# ----------------------------------------------------------------------------------------------------------------------------

	database_filenames = [path_to_database.replace(current_directory,'') for path_to_database in paths_to_databases]
	database_files = [connect(path_to_database+'/'+Recorded_Data_foldername+'/'+database_name+'.db') for path_to_database in paths_to_databases]
	...
	print('Saving '+str(len(clusters_info))+' clusters from Trials databases to overall database(s)')
	pbar = tqdm(clusters_info, unit='cluster')
	for path_to_database_index, cluster_id, cluster_datum in pbar:
		pbar.set_description('Saving '+str(cluster_id)+' from database '+str(database_filenames[path_to_database_index])+' to overall database')
		if counter == number_of_clusters_per_database+1:
			...
			print('Making '+str(small_database_name))
			db_smaller = connect(small_database_name)
		row = database_files[path_to_database_index].get(id=cluster_id)
		cluster = row.toatoms()
		...
		#	print(counter)
		counter += 1
	print('Finished Compiling all the databases across your trials together.')
	print('========================================================')
...
process_databases(number_of_clusters_per_database=number_of_clusters_per_database, sort_clusters_by=sort_clusters_by)
