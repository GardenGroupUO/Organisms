...
class Crossover:
	...
	:param crossover_type: This is the type of mating procedure the user would like to use. There are currently a few options implimented into this mating procedure:
	...
	def __init__(self,crossover_type,r_ij,vacuumAdd,size_of_clusters):
		...
		# -------------------------------------------- #
		# Assign a method to use for the mating procedure.
		if self.crossover_type == "CAS_weighted":
			...
			print("Mating Procedures available:")
	...
	def mate_Cut_and_Splice(self,parents):
		"""
		This method will perform the mating procedure as specified by the user.
		...
		first_half = parents[0][:self.half_index:]

		# Get half of second parent
		...
		if len(elementsInParent1.keys()) > 1:
			...
			second_half = first_half.deepcopy_skeleton()
			elements_in_first_half = first_half.get_elemental_makeup()
			elements_to_add_in_second_half = elementsInParent1 - elements_in_first_half
			for atom_to_add in parents[1][::-1]:
				if (elements_to_add_in_second_half[atom_to_add.symbol] > 0) and (not AtomInClusterPosition(atom_to_add,first_half)) and (not AtomInClusterPosition(atom_to_add,second_half)):
					second_half.append(atom_to_add)
					elements_to_add_in_second_half[atom_to_add.symbol] -= 1
					if not any(not value == 0 for value in elements_to_add_in_second_half.values()):
						break
		else:
			second_half = parents[1][self.half_index::]

		# Make sure that each half of the offspring is brought close to each other on each side of a plane, 
		# but do not overlap on the plane.
		#offset_z_displacement = self.displacement_of_plane(first_half,second_half)

		# Add this displacement to the second_half of the offspring
		#second_half.set_positions(second_half.get_positions() + displacement_of_second_half)

		# obtain the offspring by conbining the two clusters together
		offspring = first_half + second_half
	...
	def displacement_of_plane(self,first_half,second_half):
		"""
		...
		:param first_half: This is the first half of the offspring
		:type  first_half: GA.Cluster
		:param second_half: This is the second half of the offspring
		:type  second_half: GA.Cluster

		:returns: The z offset displacement vector to add to the second_half positions
		:rtype: numpy.array
		"""

		####### NOTE #######
		# I dont know how to get this method to work well. So will ignore doing this, and say this genetic algorithm
		# works like the brimingham parallel genetic algorithm. 
		# This method is inspired from the atomic simulation environment, ut for not it will not be implemented here
		# for now while I dont know how to do this.
		# See https://gitlab.com/ase/ase/-/blob/master/ase/ga/particle_crossovers.py
		# Method: def get_new_individual(self, parents)
		# Note: # Move the contributing apart if any distance is below blmin 
		####################

		# If the two halves are too close or too far from each other, set this half to this distance in the z axis
		z_distance_within = 0.1
	
		# 
		part1_positions = first_half.get_positions()
		part2_positions = second_half.get_positions()
		z_diff_min = float('inf')
		index1_z_diff = None
		index2_z_diff = None
		for index1 in range(len(part1_positions)):
			x1, y1, z1 = part1_positions[index1]
			for index2 in range(len(part2_positions)):
				x2, y2, z2 = part2_positions[index2]
				z_diff = z1 - z2
				if z_diff < z_diff_min:
					index1_z_diff = index1
					index2_z_diff = index2
					z_diff_min = z_dif

		#
		if z_diff_min > z_distance_within:
			zz = z_diff_min - z_distance_within
		elif 0.0 <= z_diff_min <= z_distance_within:
			zz = 0
		else:
			zz = z_diff_min - z_distance_within

		# 
		offset_z_displacement = array((0,0,-zz))
		return offset_z_displacement
	...
	def half_index_custom_method(self,cross_type):
		...
		:param crossover_type: This is the type of mating procedure the user would like to use. This should be set to "CAS_custom_XX", where XX is the percentage of parent 1 to be cut.
		:type  crossover_type: str.
