...
This class is specifically designed to hold a collection of 
clusters, such as in a population or in an offspring pool.

This class holds the foundation of the object used to store 
clusters in the population, the offspring, and other collections. 
'''
...
class Collection:
	...
	def add_metadata(self):
		...
		"""
		if self.have_database:
			# Set up details about database format.
			metadata = {}
			metadata['title'] = str(self.name)
			metadata['default_columns'] = ['name', 'cluster_energy', 'id']
			# ----------------------------------------------------
			# details about descrptions of information recorded for each cluster in the collection.
	...
	def get_cluster_names(self,order=False):
		...
		if self.write_cluster_in_RAM:
			...
			print('Issue in def get_cluster_names, in Collection.py')
			print('Can not get cluster name as they have not been recorded into the RAM memory.')
	...
	def sort_by_name(self):
		...
		if self.write_cluster_in_RAM:
			...
			print('Issue in def sort_by_name, in Collection.py')
			print('Can not get cluster name as they have not been recorded into the RAM memory, so can not sort clusters in the collection by their name.')
			import pdb; pdb.set_trace()
			exit('Finishing without completing')
	...
	def sort_by_energy(self):
		...
		if self.write_cluster_in_RAM:
			...
			print('Issue in def sort_by_energy, in Collection.py')
			print('Can not get cluster energies as clusters have not been recorded into the RAM memory, so can not sort clusters in the collection by their energies.')
			import pdb; pdb.set_trace()
			exit('Finishing without completing')
	...
	def replace(self, index, new_cluster):
		...
		"""
		# In the replace method, we first remove the old cluster at index.
		self.remove(index)
		# Then we add the new cluster into the collection into the index-th position.
	...
	def pop(self,ith):
		...
		# Remove the ith cluster from the collection.
		del self[ith]
		# return the ith cluster from the collection which has now just been removed.
		return cluster
	...
	def move_backup_database_to_normal_backup(self):
		...
		if not self.have_database:
			return
		# if the database_path exists, remove it.
		if os.path.exists(self.database_path):
			os.remove(self.database_path)
		# Change the name of the backup database file to normal backup.
		# Here, we copy the backup file and then delete rather than move it
		# We do not move directly just in case the GA is stopped midway through 
		# and become corrupt because of this.
	...
	def import_clusters_from_database_to_memory(self, current_generation, clusters_in_resumed_population, clusters_in_resumed_population_energies, decimal_place):
		...
		def load_clusters_from_database_to_RAM(database_path,current_generation,clusters_in_resumed_population,clusters_in_resumed_population_energies,decimal_place,self):
			...
			if self.check_clusters_in_database(cluster_dict, clusters_in_resumed_population, clusters_in_resumed_population_energies, decimal_place):
				...
				print('Error with loading into ram in def import_clusters_from_database_to_memory, Collection.py')
				import pdb; pdb.set_trace()
	...
	def check_database_and_determine_if_to_use_backup(self):
		...
		"""
		# Remove all the temporary database files and backup database files.
		...
		# If the current and backup databases do not exist, indicate issue. 
		...
		# If the current database does not exist, indicate issue. 
		if os.path.exists(self.database_path+'.backup.db'):
		...
		# Will check the current database and report back.
	...
	def assess_clusters_in_database(self,database_path):
		...
		"""
		# Will go through the database and make sure that it is all good 
		# and their are no issues with access data from the database.
		...
		# Check if the size of the database is the correct size
		...
		# All is go, so return a "Houston, we are go" report.
		return (True, None)
	...
	def read_collection_database(self,database_path,current_generation=None):
		...
		"""
		# Get clusters from database, and if any cluster in the database does not exist in the current generation, remove it.
		clusters = []
		cluster_to_delete = []
		with connect(database_path) as database:
			for cluster_row in database.select():
