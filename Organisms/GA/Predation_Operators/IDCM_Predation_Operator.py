...
class IDCM_Predation_Operator(Predation_Operator):
	...
	def update_LoD_database(self, collection):
		...
		data = parallelise(self.get_structural_data_method, atoms_format_clusters, processes=self.no_of_cpus)
		end_time = time.time()
	...
	def get_identical_structures_initial_population(self):
		...
		start_time = time.time()
		data = parallelise(self.check_if_two_clusters_are_identical_method, structures_to_compare, processes=self.no_of_cpus)
		end_time = time.time()
