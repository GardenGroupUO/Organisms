...
class SCM_Predation_Operator(Predation_Operator):
	...
	def __init__(self,Predation_Information,fitness_information,population,no_of_cpus,print_details): #,no_generations):
		...
		get_single_CNA_profile_method, get_cna_similarities_method = get_SCM_methods(self.SCM_Scheme)
		############################################################
		...
		self.cna_database = CNA_Database(self.rCuts,self.population,self.cut_off_similarity,get_single_CNA_profile_method,get_cna_similarities_method,no_of_cpus,self.debug)
		####################################################################################
	...
	def assess_for_violations_force_replacement(self,offspring_pool):
		...
		if not offspring_to_remove == []:
			print('Offspring removed by SCM Predation Operator (name, index in offspring pool): '+str(offspring_to_remove))
		####################################################################################################################
		if self.print_details:
