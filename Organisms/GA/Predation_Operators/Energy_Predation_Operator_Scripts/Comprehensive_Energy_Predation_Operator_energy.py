...
def check_initial_population(self,return_report=False):
	...
	"""
	########################################################################################################
	# Sort the indices of clusters in the population by energy, from lowest to highest.
	cluster_in_pop_index_energy = sorted(zip(range(len(self.population)),[cluster.energy for cluster in self.population]), key=lambda x:x[1], reverse=False)
	if (not cluster_in_pop_index_energy == []) and (not cluster_in_pop_index_energy[0][1] <= cluster_in_pop_index_energy[-1][1]):
		...
		print('cluster_in_pop_index_energy[0][1] (fitness) = '+str(cluster_in_pop_index_energy[0][1]))
		print('cluster_in_pop_index_energy[-1][1] (fitness)= '+str(cluster_in_pop_index_energy[-1][1]))
	...
	indices_of_pop_sorted_by_energy = [index for index, energy in cluster_in_pop_index_energy]
	########################################################################################################
	...
	clusters_to_remove = []
	i1 = 0
	while i1 < len(indices_of_pop_sorted_by_energy):
		index1 = indices_of_pop_sorted_by_energy[i1]; cluster1 = self.population[index1]
		i2 = i1 + 1
		while i2 < len(indices_of_pop_sorted_by_energy):
			index2 = indices_of_pop_sorted_by_energy[i2]; cluster2 = self.population[index2]
			if round(cluster2.energy - cluster1.energy,12) < self.minimum_energy_diff:
				...
				# add cluster2 to clusters_to_remove and remove it from indices_of_pop_sorted_by_energy
				clusters_to_remove.append((index_to_remove, name_to_remove))
				del indices_of_pop_sorted_by_energy[i2]
			else:
				i2 += 1
		i1 += 1
	if return_report:
...
class Cluster_Block:
	...
	def  __repr__(self):
		return str(self.collection_type)+str(self.name)+'('+str(self.index)+', energy = '+str(self.energy)+' eV)'
...
def assess_for_violations_message(swap_P_O,remove_O_similar_to_too_many_P,removal_O_P,removal_O_O):
	...
	if len(swap_P_O) > 0:
		...
		for pop_to_swap_out, off_to_swap_in in sorted(swap_P_O,key=lambda x:x[1].name,reverse=False):
			cp_removed_name = pop_to_swap_out.name; cp_removed_energy = pop_to_swap_out.energy
			off_swapped_in_name = off_to_swap_in.name; off_swapped_in_energy = off_to_swap_in.energy
			print(' --> Population Cluster removed '+str(cp_removed_name)+' (Energy: '+str(round(cp_removed_energy,6))+' eV) {Replaced with Offspring '+str(off_swapped_in_name)+' (Energy: '+str(round(off_swapped_in_energy,6))+' eV)}.')
	else:
	...
	if len(remove_O_similar_to_too_many_P) > 0:
		...
		for off_to_swap_out, geo_sim_pops in sorted(remove_O_similar_to_too_many_P,key=lambda x:x[0].name,reverse=False):
			off_removed_name = off_to_swap_out.name; off_removed_energy = off_to_swap_out.energy
			geo_sim_pop1, geo_sim_pop2 = geo_sim_pops
			geo_sim_pop1_name = geo_sim_pop1.name; geo_sim_pop1_energy = geo_sim_pop1.energy
			geo_sim_pop2_name = geo_sim_pop2.name; geo_sim_pop2_energy = geo_sim_pop2.energy
			print(' --> Offspring removed '+str(off_removed_name)+' (Energy: '+str(round(off_removed_energy,6))+' eV; energy: '+str(round(off_removed_energy,6))+') {Energetically similar to (at least) clusters in population: '+str(geo_sim_pop1_name)+' (Energy: '+str(round(geo_sim_pop1_energy,6))+' eV)} and '+str(geo_sim_pop2_name)+' (Energy: '+str(round(geo_sim_pop2_energy,6))+' eV)}.')
	else:
	...
	if len(removal_O_P) > 0:
		...
		for offspring_removed, cp_due_to in removal_O_P:
			offspring_removed_name = offspring_removed.name; offspring_removed_energy = offspring_removed.energy
			cp_due_to_name = cp_due_to.name; cp_due_to_energy = cp_due_to.energy
			print(' --> Offspring '+str(offspring_removed_name)+' (Energy: '+str(offspring_removed_energy)+' eV) {Similar to Population Cluster '+str(cp_due_to_name)+' (Energy: '+str(round(cp_due_to_energy,6))+' eV)}.')
	else:
	...
	if len(removal_O_O) > 0:
		...
		for offspring_removed, due_to_offspring in removal_O_O:
			offspring_removed_name = offspring_removed.name; offspring_removed_energy = offspring_removed.energy
			due_to_offspring_name = due_to_offspring.name; due_to_offspring_energy = due_to_offspring.energy
			print(' --> Offspring '+str(offspring_removed_name)+' (Energy: '+str(offspring_removed_energy)+'eV) {Similar to Offspring '+str(due_to_offspring_name)+' (Energy: '+str(due_to_offspring_energy)+' eV)}.')
	else:
...
def assess_for_violations_no_force_replacement(self,offspring_pool):
	...
	* They have an energy equal to another cluster in the population.
	...
	structures_to_consider.sort(key=lambda cluster: cluster.energy, reverse=False)
	index1 = 0
...
def assess_for_violations_force_replacement(self,offspring_pool):
	...
	* They have an energy equal to another cluster in the population.
	...
	structures_to_consider += [Cluster_Block(offspring_pool,'Offspring',index) for index in range(len(offspring_pool))]
	# order energies,From lowest to highest
	structures_to_consider.sort(key=lambda cluster: cluster.energy, reverse=False)
	#print([cluster.energy for cluster in structures_to_consider])
	#exit()
	if (not structures_to_consider == []) and (not structures_to_consider[0].energy <= structures_to_consider[-1].energy):
		print('Error in def assess_for_violations in class CNA_Diversity_Scheme, in Energy_Diversity_Scheme.py')
		print('The energies after the sort are the wrong way around')
		print('structures_to_consider[0].energy = '+str(structures_to_consider[0].energy))
		print('structures_to_consider[-1].energy = '+str(structures_to_consider[-1].energy))
		print('The energies need to be order from lowest to highest.')
		exit('Check this out. This program will finish without completing.')
	########################################################################################################
	if self.print_details:
		print('Start assessing for Energy Diversity')
	index_lower_energy = 0
	...
	while index_lower_energy < len(structures_to_consider):
		...
		indices_to_delete = []
		for index_higher_energy in range(index_lower_energy+1, len(structures_to_consider)):
			cluster_higher_energy = structures_to_consider[index_higher_energy]
			############################################################################################################
			energy_diff = round(cluster_higher_energy.energy - cluster_lower_energy.energy,12)
			if energy_diff < self.minimum_energy_diff:
				# The two clusters being looked at at the moment are geometrically the same. 
				if cluster_lower_energy.collection_type == 'Population':
					# if two clusters in the population are geometrically similar, we have an issue.
					if cluster_higher_energy.collection_type == 'Population':
						print('Error in def assess_for_violations in class CNA_Diversity_Scheme, in Energy_Diversity_Scheme.py')
						print('Two of the clusters in the population are identical by the max  similarity cutt off setting.')
						print('cluster_lower_energy: '+str(cluster_lower_energy))
						print('cluster_higher_energy: '+str(cluster_higher_energy))
						import pdb; pdb.set_trace()
					...
					elif cluster_higher_energy.collection_type == 'Offspring':
						removal_O_P_temp.append([cluster_higher_energy,cluster_lower_energy])
				elif cluster_lower_energy.collection_type == 'Offspring':
					...
					if cluster_higher_energy.collection_type == 'Population':
						...
						if one_instance_of_swap_P_O == []:
							one_instance_of_swap_P_O = [[cluster_higher_energy,cluster_lower_energy]]
						else:
							cluster_lower_old, cluster_higher_old = one_instance_of_swap_P_O[0]
							if not cluster_higher_old.name == cluster_lower_energy.name:
								exit('Somethings up')
							remove_O_similar_to_too_many_P_temp.append([cluster_lower_energy,[cluster_lower_old,cluster_lower]])
					...
					# remove the higher energy offspring
						...
						removal_O_O_temp.append([cluster_higher_energy,cluster_lower_energy])
				indices_to_delete.append(index_higher_energy)
			else:
				break
		################################################################################################################
		if not remove_O_similar_to_too_many_P_temp == []:
			...
			for index_higher_energy in sorted(indices_to_delete,reverse=True): # remove from highest to lowest index
				del structures_to_consider[index_higher_energy]
			index_lower_energy += 1
			################################################################################################################
	####################################################################################################################
	if self.print_details:
