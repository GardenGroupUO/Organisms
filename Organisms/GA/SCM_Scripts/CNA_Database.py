...
from Organisms.GA.SCM_Scripts.get_CNA_profile import get_CNA_profile
...
class CNA_Database:
	...
	:param get_single_CNA_profile_method: This is the method to get the CNA Profile, either from the T-SCM or the A-SCM. 
	:type  get_single_CNA_profile_method: __func__
	...
	"""
	def __init__(self,rCuts,population,cut_off_similarity,get_single_CNA_profile_method,get_similarity_profile_method,no_of_cpus,debug):
		...
		self.get_single_CNA_profile_method = get_single_CNA_profile_method
	...
	def get_details(self):
		...
		"""
		return (self.cut_off_similarity, self.get_single_CNA_method, self.get_similarity_profile_method, self.no_of_cpus, self.debug)
	...
	def add(self,collection,initialise=False):
		...
		data = get_CNA_profile(CNA_Profiles_to_make, self.get_single_CNA_profile_method, no_of_cpus=self.no_of_cpus)
		end_time = time.time()
		...
		print('make similarity profile gen')
		if initialise:
		...
		start_time = time.time()
		print('finished making similarity profile gen')
		print('make similarity profile gen')
		if self.no_of_cpus == 1:
			data = [self.get_similarity_profile_method(input_datum) for input_datum in CNA_Similarities_to_make]
		else:
			print('parallelise statement')
			data = parallelise(self.get_similarity_profile_method, CNA_Similarities_to_make, processes=self.no_of_cpus) 
		print('finished making similarity profile gen')
		end_time = time.time()
