...
def Create_An_Offspring(input_data):
	...
	(cluster_name,generation_number,population,offspring_pool_name,chance_of_mutation,r_ij,cell_length,vacuum_to_add_length,creating_offspring_mode,crossover_procedure,mutation_procedure,no_offspring_per_generation,rounding_criteria,Minimisation_Function,surface,place_cluster_where,print_details) = input_data
	toString = ''
	#if print_details:
	#	toString += "----------------------------------------------------------"+'\n'
	no_of_failed_minimisations = 0 
	no_of_explosions = 0
	...
	while True:
		...
		#######################################################################################
		if not finished_minimisation_process:
			toString += "Cluster did not minimise, the minimisation process broke. Will disregard this cluster and try mating or mutation method again"+'\n'
			no_of_failed_minimisations += 1
			continue
		...
		if not converged:
			...
			break
		# if this happens too much, probably need to address this in the code, so fail dramatically and tell the user
		if (no_of_failed_minimisations+no_of_explosions+no_of_not_converged) == 50:
			error_message = ''
			error_message += 'Issue happening at: def Create_An_Offspring, in Get_Offspring.py'+'\n'
			error_message += 'This crossover method has had a problem happen. Details below:'+'\n'
			error_message += '\t* Number of times the Minimisation_Function itself has broken: '+str(no_of_failed_minimisations)+'\n'
			error_message += '\t* Number of times that the cluster has exploded (i.e. is not a cluster but minimised into several smaller clusters): '+str(no_of_explosions)+'\n'
			error_message += '\t* Number of times the cluster minimisation did not converge: '+str(no_of_not_converged)+'\n'
			error_message += 'This has happened too much, so may mean something is wrong with either or both:'+'\n'
			error_message += '\t* Your Minimisation_Function method in your RunMinimisation.py'+'\n'
			error_message += '\t* Something is wrong with the crossover method of this program'+'\n'
			error_message += 'Talk to developers about this in the Github Issues post, or by email.'+'\n'
			error_message += 'The issues that get this GA to this point are likely rare, but the developers are very interested in these issues because it is so rare and hard to figure out what is happening, so feedback from users when this issue arises would be really helpful.'+'\n'
			print(error_message)
			print(error_message,file=sys.stderr)
			exit('This program will exit dramatically without completing.')
