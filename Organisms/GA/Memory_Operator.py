...
from Organisms.GA.SCM_Scripts.T_SCM_Methods import get_single_total_CNA_profile_method  as get_single_CNA_profile_T_SCM
from Organisms.GA.SCM_Scripts.A_SCM_Methods import get_single_atomic_CNA_profile_method as get_single_CNA_profile_A_SCM
...
class Memory_Operator:
	...
	def __init__(self,memory_operator_information):
		...
		if memory_operator_information['SCM Type'] == 'A-SCM':
			self.get_single_CNA_profile_method = get_single_CNA_profile_A_SCM
			self.get_CNA_similarity_method     = get_CNA_similarity_A_SCM
		elif memory_operator_information['SCM Type'] == 'T-SCM':
			self.get_single_CNA_profile_method = get_single_CNA_profile_T_SCM
	...
	def get_CNA_profile(self,cluster):
		...
		"""
		cluster_CNA_profile = self.get_single_CNA_profile_method(cluster,self.rCut, False)
	...
	def pop_similar_clusters_from_offspring_pool(self,collection,offspring_to_remove_because_too_similar_to_a_cluster_in_memory,print_details):
		"""
		This method is designed to remove the similar clusters from the offspring, and pop it off the offspring pool

		will return the similar clusters that have been removed from the offspring_pool.
		"""
