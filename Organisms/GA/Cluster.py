...
class Cluster(Atoms):
	...
	def sortZ(self):
		...
		"""
		# If the cluster has already been ordered by the z axis, dont need to do it again.
		if self.sorted_by_Z:
			return
		# First, get the indices to sort the z axis from largest to smallest value.
		indices_sorted_by_z = self.get_positions()[:,2].argsort()[::-1]
		# Second, reorder all the arrays in this Clusters(ase.Atoms) object so it is sorted by the z value.
		for key in self.arrays.keys():
			self.set_array(key,self.get_array(key)[indices_sorted_by_z])
		# Third, indicate the atoms in the cluster have been ordered by the z axis.
	...
	def centre_cluster_at_centre_of_cell(self,vacuumAdd):
		...
		:param vacuumAdd: The amount of value to add around the cluster.
