...
from Organisms.GA.Parallelise import parallelise
...
class GA_Program():
	...
	"""
	def __init__(self,cluster_makeup,pop_size,generations,no_offspring_per_generation,creating_offspring_mode,crossover_type,mutation_types,chance_of_mutation,r_ij,vacuum_to_add_length,Minimisation_Function,
		surface_details=None,
		epoch_settings={'use epoch': 'off'},
		cell_length='default',
		memory_operator_information={'perform_memory_operator':'Off'},
		predation_information={'Predation_Switch':"Off"},
		fitness_information={'Fitness_Switch':"Energy"},
		...
		Lock_Check_and_Set()
		# Add unlocking to the exitting procedure
	...
	def run_GA(self):
		...
		for generation_number in range(self.starting_generation,self.generations+1):
			...
			print('#'*len(generation_wording))
			pre_setup_timer_end = time()
			##############################################################################
			######################### GET OFFSPRING OPERATION ############################
			get_offsprings_timer_start = time()
			...
			clusters_being_removed_by_memory_operator = self.memory_operator.pop_similar_clusters_from_offspring_pool(self.offspring_pool,offspring_to_remove_because_too_similar_to_a_cluster_in_memory,self.print_details)
			...
			# Assign fitnesses
			self.fitness_operator.assign_all_fitnesses_before_assessing_against_predation_operator(self.offspring_pool,generation_number)
			...
			self.fitness_operator.assign_all_fitnesses_after_assessing_against_predation_operator(self.offspring_pool, generation_number, offspring_to_remove_violated_predation_operator)
			fitness_timer_2_end = time()
			##############################################################################
			############################# NATURAL SELECTION ##############################
			# Perform the natural selection procedure.
			...
			# This part of the code will perform an epoch procedure if required.
			...
			is_energy_in_fitness_function = self.fitness_operator.fitness_switch == 'Energy' or (self.fitness_operator.fitness_switch == 'Structure + Energy' and not self.fitness_operator.SCM_fitness_contribution == 1.0)
			is_SCM_in_fitness_function = self.fitness_operator.fitness_switch == 'Structure + Energy' and not self.fitness_operator.SCM_fitness_contribution == 0.0
			...
			end_time = time()
			# Timers
			pre_setup_timer = pre_setup_timer_end - pre_setup_timer_start
			get_offsprings_timer = get_offsprings_timer_end - get_offsprings_timer_start
			...
			natural_selection_timer = natural_selection_timer_end - natural_selection_timer_start
			ending_timer = ending_timer_end - ending_timer_start
			overall_time_for_full_generation = end_time - start_time
			# Print times
			print('Time Information')
			print('pre_setup_timer         = '+str(pre_setup_timer)+' seconds')
			print('get_offsprings_timer    = '+str(get_offsprings_timer)+' seconds')
			print('fitness_timer_1         = '+str(fitness_timer_1)+' seconds')
			print('memory_operator_timer_1 = '+"{:.12f}".format(memory_operator_timer_1)+' seconds')
			print('predation_timer_1       = '+str(predation_timer_1)+' seconds')
			print('fitness_timer_2         = '+str(fitness_timer_2)+' seconds')
			print('natural_selection_timer = '+str(natural_selection_timer)+' seconds')
			print('ending_timer            = '+str(ending_timer)+' seconds')
			total_time = pre_setup_timer + get_offsprings_timer + fitness_timer_1 + memory_operator_timer_1 + predation_timer_1 + fitness_timer_2 + natural_selection_timer + ending_timer
			print('DEBUG: Total time = '+str(total_time))
			print('The amount of time required for generation ' + str(generation_number) + ' is ' + str(overall_time_for_full_generation)+' seconds')
			print("----------------------------------------------------------")
	...
	def get_offsprings(self,previous_cluster_name,generation_number):
		...
		if self.no_of_cpus == 1:
			...
			offsprings = parallelise(Create_An_Offspring, tasks, processes=self.no_of_cpus)
		end_time = time()
		print('-----------------------')
