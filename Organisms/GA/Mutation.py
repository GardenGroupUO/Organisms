...
class Mutation:
	...
	def check(self, r_ij):
		...
		for mutation_type in mutation_types:
			if mutation_type.startswith('move'):
				...
				#	print('Error in def mutation, in Class MutationProcedure, in MutationProcedure.py.')
	...
	def change_mutation_chances(self):
		...
		"""
		end_point = 0.0
		mutation_types_modified_format = []
		for mutation_type, mutation_chance in self.mutation_types:
			end_point += mutation_chance
			end_point = round(end_point,12)
			mutation_information = (mutation_type, end_point)
			mutation_types_modified_format.append(mutation_information)
		self.mutation_types = tuple(mutation_types_modified_format)
